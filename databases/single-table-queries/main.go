...
func getTest() QueryNode {
	...
	proj := projectNode{[]int{0, 1}, &sort}
	lim := limitNode{5, 0, &proj}
	//sel.input = &scan
	//proj.input = &sel
	return &lim //proj
}
...
func (n *sortNode) Init() {
	n.input.Init()
	r, e := n.input.Next()
	for ; e == nil; r, e = n.input.Next() {

		n.buf = append(n.buf, r)
...
func (n *sortNode) Next() (record, error) {
	var rec record
	if n.cursor >= n.end {
		return rec, io.EOF
	}
...
func (n *limitNode) Next() (record, error) {
	if n.count >= n.limit {
		return make([]string, 0), io.EOF
	}
	rec, err := n.input.Next()
	if err == nil {
		n.count++
	}
	return rec, err
...
func (n *projectNode) Next() (record, error) {
	rec, err := n.input.Next()
	if err != nil {
		return rec, err
	}
	prec := make([]string, len(n.fields))
	c := 0
	for _, f := range n.fields {
		prec[c] = rec[f]
		c++
...
func (n *selectNode) Next() (record, error) {
...
func (n *scanNode) Init() {
	...
	n.cursor = 0
	n.rangeStart = 0
	n.rangeEnd = len(n.records)
}

func (n *scanNode) Next() (record, error) {
	if n.cursor >= n.rangeEnd {
		return nil, io.EOF
	}
	rec := n.records[n.cursor]
	n.cursor++
	return rec, nil
